...
impl EmailService {
    ...
    /// `period_end`  — end of the current billing period (UTC)
    pub async fn send_subscription_confirmation_email(
        &self,
        to: &str,
        username: &str,
        tier_name: &str,
        ...
        let subject = format!("Your {tier_name} subscription is confirmed");
        let next_billing = period_end.format("%B %-d, %Y").to_string();
        let text = format!(
            "Hi {username},\n\nThank you for subscribing to {tier_name}!\n\n\
             Your subscription is active. Next billing date: {next_billing}.\n\n\
             Manage your subscription: {base}/subscription\n\nCheers, the Magnetite team",
            base = self.base_url
        );
        let html =
            render_subscription_confirmation(username, tier_name, &next_billing, &self.base_url);
        self.send_email(to, &subject, &text, &html).await
    }

    /// Send the subscription-cancellation email.
    ///
    /// `tier_name`   — display name of the cancelled plan
    /// `period_end`  — date access expires (UTC)
    pub async fn send_subscription_cancellation_email(
        &self,
        to: &str,
        username: &str,
        tier_name: &str,
        ...
        let subject = format!("Your {tier_name} subscription has been cancelled");
        let access_expires = period_end.format("%B %-d, %Y").to_string();
        let cancelled_on = chrono::Utc::now().format("%B %-d, %Y").to_string();
        let text = format!(
            "Hi {username},\n\nYour {tier_name} subscription has been cancelled.\n\n\
             You will retain access until {access_expires}, then your account reverts to the free tier.\n\n\
             Changed your mind? Reactivate at: {base}/subscription\n\nCheers, the Magnetite team",
            base = self.base_url
        );
        let html = render_subscription_cancellation(
            username,
            tier_name,
            &cancelled_on,
            &access_expires,
...
fn render_subscription_confirmation(
    ...
    let content = format!(
        r#"
  <tr><td style="padding:50px 30px 20px 30px;text-align:center;">
    <h1 style="margin:0 0 12px 0;font-size:28px;font-weight:700;color:#ffffff;line-height:1.2;">Subscription Confirmed</h1>
    <p style="margin:0;font-size:18px;color:#10b981;font-weight:600;">{tier_name}</p>
  </td></tr>
  <tr><td style="padding:0 30px 30px 30px;text-align:center;">
    <p style="margin:0;font-size:16px;color:#9ca3af;line-height:1.6;">Hi {username}, thank you for subscribing to {tier_name}! You now have access to all features included in your plan.</p>
  </td></tr>
  <tr><td style="padding:0 30px 30px 30px;">
    <table role="presentation" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr><td style="padding:24px;background-color:#1a1a26;border-radius:12px;">
        <table role="presentation" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr><td style="padding:0 0 16px 0;border-bottom:1px solid #2a2a3a;">
            <p style="margin:0 0 4px 0;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Plan</p>
            <p style="margin:0;font-size:20px;color:#ffffff;font-weight:600;">{tier_name}</p>
          </td></tr>
          <tr><td style="padding:16px 0 0 0;">
            <p style="margin:0 0 4px 0;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Next Billing Date</p>
            <p style="margin:0;font-size:16px;color:#ffffff;">{next_billing_date}</p>
          </td></tr>
        </table>
      </td></tr>
    </table>
  ...
  <tr><td style="padding:0 30px 50px 30px;text-align:center;">
    <a href="{base_url}/subscription" style="display:inline-block;padding:16px 40px;background-color:#8b5cf6;color:#ffffff;font-size:16px;font-weight:600;text-decoration:none;border-radius:8px;">Manage Subscription</a>
  </td></tr>
"#,
        username = esc(username),
        tier_name = esc(tier_name),
        next_billing_date = esc(next_billing_date),
        base_url = base_url,
    );
    email_wrapper("Subscription Confirmed", &content, base_url)
}

fn render_subscription_cancellation(
    username: &str,
    tier_name: &str,
    cancelled_on: &str,
    access_expires: &str,
    ...
    let content = format!(
        r#"
  <tr><td style="padding:50px 30px 20px 30px;text-align:center;">
    <h1 style="margin:0 0 12px 0;font-size:28px;font-weight:700;color:#ffffff;line-height:1.2;">Subscription Cancelled</h1>
    <p style="margin:0;font-size:16px;color:#9ca3af;line-height:1.6;">Hi {username}, your subscription to {tier_name} has been cancelled.</p>
  </td></tr>
  <tr><td style="padding:0 30px 30px 30px;">
    <table role="presentation" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr><td style="padding:24px;background-color:#1a1a26;border-radius:12px;">
        <table role="presentation" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr><td style="padding:16px 0;border-bottom:1px solid #2a2a3a;">
            <p style="margin:0 0 4px 0;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Cancelled On</p>
            <p style="margin:0;font-size:16px;color:#ffffff;">{cancelled_on}</p>
          </td></tr>
          <tr><td style="padding:16px 0 0 0;">
            <p style="margin:0 0 4px 0;font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">Access Expires</p>
            <p style="margin:0;font-size:16px;color:#ffffff;">{access_expires}</p>
          </td></tr>
        </table>
      </td></tr>
    </table>
  ...
  <tr><td style="padding:0 30px 30px 30px;">
    <table role="presentation" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr><td style="padding:20px 24px;background-color:rgba(245,158,11,0.1);border-radius:8px;border-left:3px solid #f59e0b;">
        <p style="margin:0 0 8px 0;font-size:14px;color:#f59e0b;font-weight:600;">What happens next?</p>
        <p style="margin:0;font-size:14px;color:#9ca3af;line-height:1.6;">You will retain access to your subscription features until {access_expires}. After that, your account will revert to the free tier.</p>
      </td></tr>
    </table>
  </td></tr>
  <tr><td style="padding:0 30px 30px 30px;text-align:center;">
    <p style="margin:0 0 20px 0;font-size:16px;color:#9ca3af;">Changed your mind?</p>
    <a href="{base_url}/subscription" style="display:inline-block;padding:16px 40px;background-color:#8b5cf6;color:#ffffff;font-size:16px;font-weight:600;text-decoration:none;border-radius:8px;">Reactivate Subscription</a>
  </td></tr>
  <tr><td style="padding:0 30px 50px 30px;text-align:center;">
    <p style="margin:0;font-size:14px;color:#6b7280;">Need help? Contact us at <a href="mailto:support@magnetite.gg" style="color:#8b5cf6;text-decoration:none;">support@magnetite.gg</a></p>
  </td></tr>
"#,
        username = esc(username),
        tier_name = esc(tier_name),
        cancelled_on = esc(cancelled_on),
        access_expires = esc(access_expires),
        base_url = base_url,
    );
    email_wrapper("Subscription Cancelled", &content, base_url)
}
...
mod tests {
    ...
    #[test]
    fn test_render_subscription_confirmation_contains_fields() {
        let html =
            render_subscription_confirmation("bob", "Pro", "June 29, 2026", "http://localhost");
        assert!(html.contains("Pro"));
        assert!(html.contains("bob"));
        assert!(html.contains("June 29, 2026"));
        assert!(html.contains("http://localhost/subscription"));
    }

    #[test]
    fn test_render_subscription_cancellation_contains_fields() {
        let html = render_subscription_cancellation(
            "carol",
            "Basic",
            "May 30, 2026",
        ...
        assert!(html.contains("carol"));
        assert!(html.contains("May 30, 2026"));
        assert!(html.contains("June 29, 2026"));
    }
}
