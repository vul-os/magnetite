...
// ─────────────────────────────────────────────────────────────────────────────
// 7. Wise IBAN validation
// ─────────────────────────────────────────────────────────────────────────────

#[cfg(test)]
mod wise_iban_tests {
    use magnetite_backend::services::wise::{RecipientDetails, WiseClient};

    /// Build a sandbox WiseClient via env vars (mirrors the pattern in wise.rs inline tests).
    fn sandbox_client() -> WiseClient {
        // Set WISE_SANDBOX=true so WiseClient::from_env() activates sandbox mode.
        unsafe {
            std::env::set_var("WISE_SANDBOX", "true");
        }
        WiseClient::from_env()
    }

    #[test]
    fn recipient_details_iban_fields_accept_valid_iban() {
        let details = RecipientDetails {
            account_holder_name: "Alice Rust".to_string(),
        ...
        assert_eq!(details.iban.as_deref(), Some("DE89370400440532013000"));
        assert_eq!(details.bic.as_deref(), Some("COBADEFFXXX"));
    }

    #[test]
    fn recipient_details_iban_minimum_length() {
        // IBAN is always at least 15 characters (smallest country format).
        let iban = "GB82WEST12345698765432";
        assert!(iban.len() >= 15, "IBAN must be at least 15 chars");
    }

    #[test]
    fn recipient_details_bic_is_optional() {
        let details = RecipientDetails {
            ...
            bic: None, // BIC is optional for SEPA
        };
        assert!(details.bic.is_none(), "BIC should be optional for SEPA");
        assert!(details.iban.is_some());
    }

    #[tokio::test]
    async fn sandbox_create_recipient_with_iban_returns_sandbox_id() {
        let client = sandbox_client();
        let details = RecipientDetails {
            account_holder_name: "Alice SEPA".to_string(),
            country: "DE".to_string(),
            currency: "EUR".to_string(),
            account_type: None,
            routing_number: None,
        ...
        let result = client.create_recipient(&details).await;
        assert!(
            result.is_ok(),
            "sandbox IBAN recipient creation must succeed"
        );
        let id = result.unwrap();
        assert!(
            id.starts_with("sandbox_recipient_"),
            "sandbox id must have sandbox prefix, got: {id}"
        );
    }

    #[tokio::test]
    async fn sandbox_create_recipient_email_type_returns_sandbox_id() {
        let client = sandbox_client();
        let details = RecipientDetails {
            account_holder_name: "Bob Email".to_string(),
        ...
        assert!(
            result.is_ok(),
            "sandbox EMAIL recipient creation must succeed"
        );
        let id = result.unwrap();
        assert!(id.starts_with("sandbox_recipient_"));
    }

    #[tokio::test]
    async fn sandbox_create_recipient_ach_type_returns_sandbox_id() {
        let client = sandbox_client();
        let details = RecipientDetails {
            account_holder_name: "Carol ACH".to_string(),
            country: "US".to_string(),
            currency: "USD".to_string(),
        ...
        assert!(
            result.is_ok(),
            "sandbox ACH recipient creation must succeed"
        );
        let id = result.unwrap();
        assert!(id.starts_with("sandbox_recipient_"));
    }
    ...
    #[tokio::test]
    async fn unconfigured_client_create_recipient_errors_with_clear_message() {
        // Without WISE_API_TOKEN or WISE_SANDBOX=true, create_recipient must return Err.
        // Build the client before clearing env vars (env change is process-wide).
        let saved_token = std::env::var("WISE_API_TOKEN").ok();
        let saved_profile = std::env::var("WISE_PROFILE_ID").ok();
        let saved_sandbox = std::env::var("WISE_SANDBOX").ok();

        unsafe {
            std::env::remove_var("WISE_API_TOKEN");
            std::env::remove_var("WISE_PROFILE_ID");
            std::env::remove_var("WISE_SANDBOX");
        }

        let client = WiseClient::from_env();
        let details = RecipientDetails {
            account_holder_name: "Test".to_string(),
            country: "US".to_string(),
            currency: "USD".to_string(),
            account_type: None,
            routing_number: None,
        ...
        // Restore env before asserting so failures don't leave env dirty.
        unsafe {
            if let Some(v) = saved_token {
                std::env::set_var("WISE_API_TOKEN", v);
            }
            if let Some(v) = saved_profile {
                std::env::set_var("WISE_PROFILE_ID", v);
            }
            if let Some(v) = saved_sandbox {
                std::env::set_var("WISE_SANDBOX", v);
            }
        }

        assert!(result.is_err(), "unconfigured client must return Err");
        let err = result.unwrap_err().to_string();
        assert!(
            err.contains("payouts not configured"),
            "error must mention 'payouts not configured': {err}"
        );
    }

    #[test]
    fn recipient_details_serializes_without_empty_optional_fields() {
        // skip_serializing_if = None fields must not appear in JSON.
        let details = RecipientDetails {
            account_holder_name: "Dave".to_string(),
            country: "US".to_string(),
            currency: "USD".to_string(),
            account_type: None,
            routing_number: None,
        ...
        assert!(
            !json.contains("\"iban\":null"),
            "iban: None must not serialize as null"
        );
        assert!(
            !json.contains("\"bic\":null"),
            "bic: None must not serialize as null"
        );
        assert!(json.contains("dave@example.com"));
    }

    #[test]
    fn recipient_details_with_iban_serializes_iban_field() {
        let details = RecipientDetails {
            account_holder_name: "Eve".to_string(),
            country: "DE".to_string(),
            currency: "EUR".to_string(),
            account_type: None,
            routing_number: None,
        ...
        let json = serde_json::to_string(&details).expect("serialize");
        assert!(
            json.contains("DE89370400440532013000"),
            "iban must be present in JSON"
        );
        assert!(json.contains("COBADEFFXXX"), "bic must be present in JSON");
    }
}
