 ...
 *
 * Why this exists: two real failures across this programme would both have
 * been caught here.
 ...
 *
 * ---------------------------------------------------------------------------
 * This is a per-repo copy of the canonical script that originated in
 * gitstate/web/scripts/check-lint-config.mjs. Everything that varies per
 * repo lives in the CONFIG block directly below. See that canonical copy's
 * footer comment for what a sibling repo is expected to change.
 * ---------------------------------------------------------------------------
 */
...
const CONFIG = {
  ...
  // resolves. magnetite's web/tsconfig.json has
  // `"include": ["src", "magnetite-web-client/src", "e2e"]` — "src" is
  // covered and is where the app's own TS-aware eslint.config.js block
  // (files: ['src/**/*.{ts,tsx}']) applies recommendedTypeChecked.
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). This reaches
  // outside webRoot on purpose — magnetite has package.json files in several
  // sibling crates/tools (magnetite-cli, magnetite-e2e, etc.) alongside the
  // many Rust crates, and a stray range on any of them is worth catching even
  // though only web/ actually depends on typescript today.
  ...
  // (assertion C). Measured real count on 2026-08-05: 346 files, 0 errors,
  // 32 warnings (before this script itself was added). 250 leaves headroom
  // for normal churn (files added/removed/moved) while still catching a
  // broken glob or an accidentally-scoped `files:` pattern, which would drop
  // the count by dozens or hundreds, not single digits.
  coverageFloor: 250,
}
