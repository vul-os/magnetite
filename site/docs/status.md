...
| Web-bundle hosting (rung 0) | `magnetite-web-host` serves a Godot / Unity / three.js web build as a content-addressed package: COOP+COEP, precompressed `.br`/`.gz`, range requests, per-file BLAKE3 verification that fails closed, and an entitlement gate. Needs no central service, no coordinator and no chain. `crossOriginIsolated` and `SharedArrayBuffer` are confirmed in a real headless Chromium, with a negative control. **No real engine export has been served through it** — the tests use a fixture that mimics the Godot 4 export file layout, and the entitlement gate has only ever run against `MockPaymentRail`. See [Web bundles](./docs.html#web-bundles). | <span class="mg-s live">Running</span> |
...
| Capability | What it is | State |
|---|---|---|
| On-chain payments | The `PaymentRail` seam models atomic wallet-to-wallet splits, signed-receipt entitlements, hosting channels and wager escrow. What ships by default is `MockPaymentRail`, a deterministic offline mock that signs receipts so CI can run without a network. An optional, feature-gated `magnetite-solana-rail` also exists and can be selected with `PAYMENT_RAIL=solana --features solana`, but no real payment has ever settled through either rail as wired into this backend. | <span class="mg-s mock">Mock only</span> |
| Stellar payment rail | `magnetite-stellar-rail` — a second, standalone `PaymentRail` implementation for native USDC on Stellar, with its own Ed25519 signing, XDR transaction construction and Horizon client. It is implemented and has tests (70 offline tests against a scripted fake Horizon; see the crate's own "HONEST STATUS" doc comment). **It is not wired into the backend**: `PAYMENT_RAIL` (`backend/src/services/payment.rs`) accepts only `"mock"` or `"solana"` today — there is no `"stellar"` value, so this rail cannot be selected by the running server at all. No payment has ever settled through this crate's own code, live or otherwise; the one real Stellar testnet settlement anywhere in the suite (single-leg, 2026-07-30) belongs to a sibling implementation in the separate `patala` repo (`patala-stellar`), which does not share code with this crate and does not make this crate's Horizon path proven. Mainnet, multi-party splits, the Solana rail, fiat adapters and Hyperswitch remain unverified everywhere. | <span class="mg-s no">Implemented, unwired</span> |
