...
This page documents Magnetite's security model. The **authentication model**,
**network security** and **hardening checklist** sections below describe the
pre-redesign REST backend (JWT, PostgreSQL, Redis — see
[Self-hosting](#self-hosting)). **Game sandboxing** and
**[Signature verification](#sec-signature-verification-the-fail-closed-core)**
describe the decentralized core itself — the WASM sandbox, content-addressed
games, and the three fail-closed signature checks — and apply regardless of
which surface you run.
...
- [ ] TLS 1.2+ is enforced on all public endpoints (see `docs/self-hosting/ssl.md`)
...
- [ ] Automated backups configured (see `docs/self-hosting/database.md`)
...
See `docs/self-hosting/ssl.md` for Let's Encrypt automation.
