...
| A21 | Node deploy recipe distinct from the legacy backend's (`fly.toml` targets the old central backend) — **2026-07-31: written and verified** (`docs/hosting-a-server.md` "Deploying a node in production"). Confirmed by reading code, not assumed: `fly.toml`/`Dockerfile.fly` build `magnetite-backend` (Postgres + Redis + ~30 route modules, `/health`, `/metrics`) — a genuinely different product than a node. Two node-shaped binaries exist with different feature sets: `magnetite-serve` (`magnetite-runtime`, has a Dockerfile, no identity/tracker/cluster, already proven via A19's Caddy recipe) and `magnetite node` (`magnetite-cli`, has identity/tracker/cluster, **no Dockerfile exists for it anywhere in this repo**). Recipe given for the latter — systemd unit + firewall table + durability notes, fronted by A19's already-verified Caddy config, matching flowstock's/pango's `CLOUD-NODE.md` "binary + systemd" shape rather than a fourth. **Verified live this session**: booted the CLI's prebuilt release binary against an existing compiled `.wasm`, got the real capacity/identity banner, a real `LISTEN` socket (`lsof`), and `node.key` materializing at `0600` — pasted verbatim in the doc. **Not verified**: systemd itself (no systemd on this macOS host, so the exact unit file is written, not exercised), and the CLI-node + Caddy combination was not chained as one continuous test (each half is separately proven: A19 proved Caddy+`magnetite-serve`, this pass proved the CLI's `node` boots/binds/persists identity). No untested cluster YAML authored, per the explicit instruction not to | **[ADDRESSED — recipe written, node boot verified live; systemd + the full combined chain unexercised]** |
| A22 | Document the self-hostable tracker as the zero-third-party path — **2026-07-31: documented honestly, and the honest answer is narrower than the framing assumed** (`docs/zero-third-party-path.md`). LAN-only play needs zero external service (`LanDiscovery` is the default, verified: no `TRACKER_URL` ⇒ `None` in `magnetite-runtime/src/tracker.rs`), and most self-hosters need no tracker at all — you already know your own domain. But **the tracker is not actually self-hostable as a lightweight artifact today**: the only server-side implementation of `/api/v1/discovery/*` lives inside the ~30-route `magnetite-backend` monolith and needs Postgres (`discovery_ads` is in the single baseline migration alongside every other table) — verified by reading `backend/src/main.rs` and the migration directly. `discovery::router(pool: PgPool)` is `pub` and only needs a Postgres pool, so a minimal standalone tracker is architecturally possible, but **has not been built or tested anywhere in this repo** — that gap is stated plainly, not glossed. A real security nuance surfaced in the process: `verify_announce` (signature check) runs on write only; the read path (`HttpTrackerClient::find`/`parse_sessions`) never re-verifies a signature, so a naive minimal-tracker reimplementation that skipped write-side verification would let anyone spoof a session ad — flagged for whoever eventually builds that extraction. Reachability and package-distribution boundaries also mapped, each claim URL'd (Let's Encrypt, WireGuard/Tailscale/Headscale, Syncthing's own default relay/discovery servers — sourced from Syncthing's own docs, since "just use Syncthing" quietly reintroduces a third party unless explicitly disabled) | **[DOCUMENTED — honest boundary drawn; a lightweight standalone tracker itself remains unbuilt]** |
