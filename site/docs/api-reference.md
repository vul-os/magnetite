...
This is the REST API of the pre-redesign backend (`backend/`, served at
`/api/v1/` on port 8080 — see [Self-hosting](#self-hosting)), not the
decentralized `magnetite` node's wire protocol (that's `ServerNet`/
`ClientNet` over WebSocket — see [Architecture](#architecture)).
...
platform holds no funds. See [Payments](#payments).
...
`payment_receipts` row (see [Payments](#payments)). The
...
the signed receipt and revokes the entitlement — see `docs/refunds.md` in the repo.
...
Documented in full in `docs/for-developers/marketplace.md` in the repo
and [Economy & Marketplace](#economy-marketplace).
...
See [Economy & Marketplace](#economy-marketplace).
