...
> **Legacy backend surface.** The deployment/database/OAuth issues below are
> about the pre-redesign REST backend (`backend/`, PostgreSQL + Redis) — see
> [Self-hosting](#self-hosting). If you're running the decentralized
> `magnetite` node instead, its failure modes are covered in
> [Hosting a server](#hosting-a-server). The **Payment Problems** section
> below applies to both: payments always route through the same
> non-custodial `PaymentRail` seam.
...
See `docs/self-hosting/troubleshooting.md` in the repo (payment-issues section)
