...
> **Legacy backend surface.** This is part of the pre-redesign REST backend
> (`backend/`, PostgreSQL) — see [Self-hosting](#self-hosting). The `payouts`
> category name is a holdover from before the non-custodial redesign; there
> are no payouts, as the table below notes.
