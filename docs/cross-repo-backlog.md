...
| A12 | Retire `magnetite-solana-rail` **after** the Stellar rail lands — port the ten fail-closed checks and the scripted-fake-RPC pattern first, then delete rather than leave feature-gated | **[DECIDED]** |
...
| A19 | `wss://` / TLS on the node — mixed-content blocking means an `https://` page cannot open `ws://`, so this gates **all** browser games at R1. rustls + ACME, proxy recipe, or tunnel | **[BLOCKER for R1]** |
...
| A23 | Evaluate FlowStock's **folder transport** (`docs/SYNC.md` §Folder sync) for package and replay-log distribution — content-addressed packages need no live socket | **[OPEN]** |
...
| A26 | Verify Stellar history retention — *not* a gate (self-proving receipts retire it), but worth knowing | **[OPEN]** |
