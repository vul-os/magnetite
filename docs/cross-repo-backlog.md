...
| A25 | TRACT-shaped storefront replacing the custodial `marketplace.rs` surface (~36 legacy API modules) | **[OPEN]** |
| A26 | Verify Stellar history retention — *not* a gate (self-proving receipts retire it), but worth knowing. **Assessed** (`docs/stellar-history-retention.md`, primary sources with URLs): `patala-stellar::verify`'s steps 1–6 (signature/arithmetic self-consistency) are offline and survive any retention loss forever; only step 7 (Horizon confirms the tx and its envelope) degrades, and it fails **closed** (a clean 404 ⇒ `Ok(false)`, a false negative, never a false accept). Concretely for today's testnet tx (`32663937fe…`, ledger `3882739`, `horizon-testnet.stellar.org`): testnet does a **full reset to genesis 2–4×/year** (Stellar Core + Horizon + Stellar RPC together) — a harder deadline than mainnet's rolling one-year Horizon truncation. **Found in the process, not previously known**: the A14 `Settlement::Settled`/`SignedUnsettled` split that is *supposed* to make this a non-gate is not wired into any real rail yet — `magnetite-solana-rail` does not override `verify_receipt_for_item_tiered` despite `payment.rs`'s doc comment claiming it does (stale claim, corrected), and the in-progress `magnetite-stellar-rail` likewise does not as of this read — so today a Horizon miss (pruned or reset) reads as an outright refusal, not as "signed but unconfirmed." Decision needed: wire a rail's Horizon-404 case to `SignedUnsettled` before this is genuinely retired as a non-issue | **[OPEN] — assessed, wiring decision needed** |
