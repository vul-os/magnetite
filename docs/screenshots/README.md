# docs/screenshots

> **Superseded by [`docs/screenshots.md`](../screenshots.md).** This file is
> an earlier draft of the same gallery, kept only as the directory-local note;
> its file/status tables have drifted from the maintained version (e.g. the
> app screenshots below are pre-redesign filenames). Not linked from any
> index on purpose — the current chapter is `docs/screenshots.md`.

Generated by `npm run screenshotter` (alias `npm run screenshots`; script:
`scripts/screenshots.mjs`). Everything is captured in **light and dark** at
retina (1440×900 @2x) with **no backend, database, or wasm build** required.

Two surfaces are captured:

1. The static marketing site (`site/index.html`) and docs viewer
   (`site/docs.html`), served from a tiny built-in Node http server.
2. The React app (`src/`), booted on a throwaway `vite` dev server with
   `VITE_USE_MOCKS=true` so every hook serves deterministic mock data.

## Static site

| File | Surface | Status |
|------|---------|--------|
| screenshots/landing-light.png | Landing page — hero, features, seams, quick start | populated |
| screenshots/docs-overview-light.png | Docs — Overview | populated |
| screenshots/architecture-light.png | Docs — Architecture (node / discovery / payment / comms diagram) | populated |
| screenshots/hosting-a-server-light.png | Docs — Hosting a server | populated |
| screenshots/payments-light.png | Docs — Payments | populated |
| screenshots/comms-light.png | Docs — Comms | populated |
| screenshots/landing-dark.png | Landing page — hero, features, seams, quick start | populated |
| screenshots/docs-overview-dark.png | Docs — Overview | populated |
| screenshots/architecture-dark.png | Docs — Architecture (node / discovery / payment / comms diagram) | populated |
| screenshots/hosting-a-server-dark.png | Docs — Hosting a server | populated |
| screenshots/payments-dark.png | Docs — Payments | populated |
| screenshots/comms-dark.png | Docs — Comms | populated |

## App

| File | Surface | Status |
|------|---------|--------|
| screenshots/app-servers-light.png | App — Server browser (discovered SessionAds, bring-your-own-server) | populated |
| screenshots/app-wallet-light.png | App — Non-custodial wallet (linked address + signed receipts) | populated |
| screenshots/app-marketplace-light.png | App — Game catalog (content-addressed games) | populated |
| screenshots/app-game-light.png | App — Game detail | populated |
| screenshots/app-developers-light.png | App — Developer dashboard | populated |
| screenshots/app-earnings-light.png | App — Developer revenue (receipt-backed) | populated |
| screenshots/app-servers-dark.png | App — Server browser (discovered SessionAds, bring-your-own-server) | populated |
| screenshots/app-wallet-dark.png | App — Non-custodial wallet (linked address + signed receipts) | populated |
| screenshots/app-marketplace-dark.png | App — Game catalog (content-addressed games) | populated |
| screenshots/app-game-dark.png | App — Game detail | populated |
| screenshots/app-developers-dark.png | App — Developer dashboard | populated |
| screenshots/app-earnings-dark.png | App — Developer revenue (receipt-backed) | populated |

To regenerate: `npm run screenshotter`
