...
export default function Games() {
  ...
  const developers = useMemo(() => {
    return [...new Set(MOCK_GAMES.map(g => g.developer))].sort();
  }, []);
  ...
  const filteredGames = useMemo(() => {
    ...
    if (developerFilter) {
      games = games.filter(g => g.developer === developerFilter);
    }
        ...
        <main className="admin-main">
          <header className="admin-header">
            <div>
              ...
              <p>Review and manage platform games</p>
            </div>
            {pendingCount > 0 && (
              ...
              <tbody>
                {paginatedGames.map(game => (
                  <tr key={game.id}>
                    ...
                    <td><span className={getStatusBadge(game.status)}>{game.status}</span></td>
                    <td>
                      {game.featured ? (
