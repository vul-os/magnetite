...
import type { VoiceRoom, VoiceParticipant, ActionResult } from '../types/comms';
...
export function useVoice(communityId: string | null) {
  const [rooms, setRooms] = useState<VoiceRoom[]>(USE_MOCKS ? MOCK_ROOMS : []);
  ...
  const fetchRooms = useCallback(async () => {
    ...
    try {
      ...
      if (!cancelled) {
        setRooms(Array.isArray(data) ? data : ((data as { rooms?: VoiceRoom[] } | null)?.rooms ?? []));
      }
    } catch (err) {
      if (!cancelled) {
        setError(errMessage(err, 'Failed to load voice rooms'));
        setRooms([]);
      }
    } finally {
   ...
   */
  const joinRoom = useCallback(async (roomId: string): Promise<ActionResult> => {
    ...
    try {
      ...
      let tokenData: { token: string; room_id?: string };
      if (USE_MOCKS) {
        tokenData = { token: `mock-voice-token-${roomId}-${Date.now()}`, room_id: roomId };
      } else {
        tokenData = await api.voice.joinToken(roomId) as { token: string; room_id?: string };
      }
      ...
      const message = errMessage(err, 'Failed to join voice room');
      setError(message);
  ...
  const updateParticipant = useCallback((userId: string, updates: Partial<VoiceParticipant>) => {
    setParticipants((prev) =>
