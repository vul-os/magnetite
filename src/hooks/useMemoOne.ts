...
function depsMatch(prevDeps: readonly unknown[] | null, nextDeps: readonly unknown[] | null): boolean {
  if (!prevDeps || !nextDeps) return false;
  if (prevDeps.length !== nextDeps.length) return false;
  return prevDeps.every((dep, i) => Object.is(dep, nextDeps[i]));
}

interface MemoOneCache<T> {
  deps: readonly unknown[] | null;
  result: T | null;
  computed: boolean;
}

export function useMemoOne<T>(fn: () => T, deps: readonly unknown[]): T {
  ...
  const ref = useRef<MemoOneCache<T>>({ deps, result: null, computed: false });
